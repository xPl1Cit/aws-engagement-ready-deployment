pipeline {
    ...
    parameters {
        ...
        string(name: 'ENVIRONMENT', defaultValue: 'test', description: 'Build environment')
        string(name: 'DEPLOYMENT_COLOR', defaultValue: 'blue', description: 'Deployment Color')
    }
            ...
            steps {
                withCredentials([
                    ...
                    sh """
                        ...
                        chmod +x ./k8s/deploy-service.sh
                        ./k8s/deploy-service.sh ${params.DEPLOYMENT_COLOR}
                        chmod +x ./k8s/deploy-autoscaling.sh
